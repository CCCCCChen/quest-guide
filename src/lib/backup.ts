...
function getAllBackupKeys(): string[] {
  ...
  try {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k && (k.startsWith(BACKUP_KEY_PREFIX) || k.startsWith(DAILY_BACKUP_PREFIX))) {
        keys.push(k);
      }
    }
  } catch {
  ...
  try {
    ...
    if (opBackups.length > MAX_OPERATION_BACKUPS) {
      ...
      for (const b of toRemove) {
        scopedStorage.removeItem(`${BACKUP_KEY_PREFIX}${b.id}`);
      }
    }
    ...
    if (dailyBackups.length > MAX_DAILY_BACKUPS) {
      ...
      for (const b of toRemove) {
        scopedStorage.removeItem(`${DAILY_BACKUP_PREFIX}${b.id}`);
      }
    }
  } catch {
    // ignore
...
export function createBackup(reason?: string, forceDaily = false): IBackupInfo | null {
  try {
    ...
    scopedStorage.setItem(key, JSON.stringify(snapshot));
    pruneOldBackups();
    return {
      id: snapshot.id,
...
export function restoreBackup(backupId: string): boolean {
  try {
    ...
    for (const key of Object.keys(target.data)) {
      scopedStorage.setItem(key, target.data[key]);
    }
