        ...
        if (importMode === 'merge') {
          ...
          if (existing && (key === '__quest_guild_tasks' || key === '__quest_guild_shop_items' || key === '__quest_guild_redemptions' || key === '__quest_guild_focus_log')) {
            try {
              ...
              const newArr = JSON.parse(value) as unknown[];
              const existingIds = new Set(existingArr.map((item: { id?: string }) => item.id));
              const merged = [...existingArr, ...newArr.filter((item: { id?: string }) => item.id && !existingIds.has(item.id))];
